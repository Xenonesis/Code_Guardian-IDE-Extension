...
const securityAnalysis_1 = require("./analysis/securityAnalysis");
const codeQuality_1 = require("./analysis/codeQuality");
const secretDetection_1 = require("./analysis/secretDetection");
const workspaceScanner_1 = require("./analysis/workspaceScanner");
const lineHighlighter_1 = require("./analysis/lineHighlighter");
const enhancedWebview_1 = require("./dashboard/enhancedWebview");
const chatbotWebview_1 = require("./dashboard/chatbotWebview");
const enhancedTreeViewProvider_1 = require("./dashboard/enhancedTreeViewProvider");
const statusBarManager_1 = require("./dashboard/statusBarManager");
const notificationManager_1 = require("./dashboard/notificationManager");
const settingsPanel_1 = require("./dashboard/settingsPanel");
...
function activate(context) {
    ...
    function getAiProvider() {
        if (!aiProvider)
            aiProvider = new aiProvider_1.AiProvider();
        return aiProvider;
    }
    function getSecurityAnalysis() {
        if (!securityAnalysis)
            securityAnalysis = new securityAnalysis_1.SecurityAnalysis();
        return securityAnalysis;
    }
    function getCodeQuality() {
        if (!codeQuality)
            codeQuality = new codeQuality_1.CodeQuality();
        return codeQuality;
    }
    function getSecretDetection() {
        if (!secretDetection)
            secretDetection = new secretDetection_1.SecretDetection();
        return secretDetection;
    }
    // Initialize core components only
    treeProvider = new enhancedTreeViewProvider_1.EnhancedCodeGuardianTreeProvider();
    ...
    function getEnhancedWebview() {
        if (!enhancedWebview)
            enhancedWebview = new enhancedWebview_1.EnhancedWebview(context);
        return enhancedWebview;
    }
    function getSettingsPanel() {
        if (!settingsPanel)
            settingsPanel = new settingsPanel_1.SettingsPanel(context);
        return settingsPanel;
    }
    function getChatbot() {
        if (!chatbot)
            chatbot = new chatbot_1.GuardianChatbot(context);
        return chatbot;
    }
    function getChatbotWebview() {
        if (!chatbotWebview)
            chatbotWebview = new chatbotWebview_1.ChatbotWebview(context, getChatbot());
        return chatbotWebview;
    }
    function getWorkspaceScanner() {
        if (!workspaceScanner) {
            workspaceScanner = new workspaceScanner_1.WorkspaceScanner();
    ...
    function getLineHighlighter() {
        if (!lineHighlighter) {
            lineHighlighter = new lineHighlighter_1.LineHighlighter();
    ...
    vscode.workspace.onDidSaveTextDocument(async (document) => {
        const config = vscode.workspace.getConfiguration('guardianSecurity');
        if (!config.get('analysisOnSave', true))
            return;
        const editor = vscode.window.activeTextEditor;
    ...
    function hasSignificantChanges(oldContent, newContent) {
        if (!oldContent)
            return true;
    ...
    async function runAutoAnalysis(editor) {
        try {
            setAnalyzing(true);
            ...
            const [vulnerabilities, qualityMetrics, secrets, suggestions] = await Promise.all([
                (0, performanceMonitor_1.measureAnalysis)('auto-security', () => getSecurityAnalysis().analyzeSecurity(code)),
                (0, performanceMonitor_1.measureAnalysis)('auto-quality', () => getCodeQuality().analyzeQuality(code)),
                (0, performanceMonitor_1.measureAnalysis)('auto-secrets', () => getSecretDetection().detectSecrets(code)),
                (0, performanceMonitor_1.measureAnalysis)('auto-ai', () => getAiProvider().getSuggestions(code))
            ]);
            ...
            const criticalIssues = vulnerabilities.filter((v) => v.includes('[CRITICAL]') || v.includes('[HIGH]')).length + secrets.filter((s) => s.includes('[CRITICAL]') || s.includes('[HIGH]')).length;
            if (criticalIssues > 0) {
    ...
    function updateLineHighlights(document, data) {
        const highlights = [];
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('codeGuardian.analyzeSecurity', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        try {
            ...
            const code = editor.document.getText();
            const vulnerabilities = await (0, performanceMonitor_1.measureAnalysis)('security', () => getSecurityAnalysis().analyzeSecurity(code));
            ...
            if (vulnerabilities.length > 0) {
                const criticalVulns = vulnerabilities.filter((v) => typeof v === 'string' && (v.includes('injection') || v.includes('XSS')));
            ...
            setAnalyzing(false);
            await notificationManager.showAnalysisError('security', error);
        }
        ...
        try {
            ...
            const code = editor.document.getText();
            const qualityMetrics = await (0, performanceMonitor_1.measureAnalysis)('quality', () => getCodeQuality().analyzeQuality(code));
            ...
            setAnalyzing(false);
            await notificationManager.showAnalysisError('quality', error);
        }
        ...
        try {
            ...
            const code = editor.document.getText();
            const secrets = await (0, performanceMonitor_1.measureAnalysis)('secrets', () => getSecretDetection().detectSecrets(code));
            ...
            setAnalyzing(false);
            await notificationManager.showAnalysisError('secrets', error);
        }
        ...
        try {
            ...
            setAnalyzing(false);
            await notificationManager.showAnalysisError('ai', error);
        }
        ...
        try {
            ...
            if (totalIssues === 0) {
                vscode.window.showInformationMessage(`✅ Workspace analysis complete! Scanned ${filesScanned} files - No security issues found. Quality: ${avgQualityScore}%\n\n📊 Results dashboard opened automatically.`);
            }
            else {
                vscode.window.showWarningMessage(`⚠️ Workspace analysis complete! Scanned ${filesScanned} files - Found ${totalIssues} issues in ${filesWithIssues} files. Quality: ${avgQualityScore}%\n\n📊 Results dashboard opened automatically.`);
            }
        }
        catch (error) {
            setAnalyzing(false);
        ...
        try {
            ...
            const analysisResults = await Promise.allSettled([
                Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Security analysis timeout')), 10000))
                ]),
                Promise.race([
                    getCodeQuality().analyzeQuality(code),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Quality analysis timeout')), 10000))
                ]),
                Promise.race([
                    getSecretDetection().detectSecrets(code),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Secret detection timeout')), 10000))
                ]),
                Promise.race([
                    getAiProvider().getSuggestions(code),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('AI suggestions timeout')), 10000))
                ])
            ]);
    ...
    vscode.commands.registerCommand('codeGuardian.openChatbot', () => {
        ...
        if (Object.keys(analysisData).length === 0) {
            try {
                ...
                setAnalyzing(false);
                await notificationManager.showAnalysisError('comprehensive', error);
                return;
            }
        }
    ...
    vscode.commands.registerCommand('codeGuardian.scanWorkspace', async () => {
        try {
            ...
            const totalIssues = results.reduce((sum, result) => sum + result.vulnerabilities.length + result.secrets.length + result.qualityIssues.length, 0);
            ...
            vscode.window.showInformationMessage(`Workspace scan complete! Found issues in ${results.length} files (${totalIssues} total issues).`, 'View Results').then(selection => {
            ...
            await notificationManager.showAnalysisError('workspace', error);
        }
    ...
    vscode.commands.registerCommand('codeGuardian.clearAllCaches', () => {
        try {
            ...
            performanceMonitor_1.perfMonitor.clearMetrics();
    ...
    vscode.commands.registerCommand('codeGuardian.smartAnalysis', async () => {
        ...
        if (!editor) {
            ...
            const action = await vscode.window.showInformationMessage('🛡️ Guardian Security: No file is currently open for analysis.', 'Open File', 'Open Recent', 'Scan Workspace', 'Cancel');
            switch (action) {
                case 'Open File':
                    // Open file dialog
                ...
                case 'Open Recent':
                    ...
                    setTimeout(() => {
                        vscode.window.showInformationMessage('💡 After opening a file, run "Guardian Security: Run All Analysis" to analyze it.', 'Got it!');
                    }, 1000);
                    break;
                case 'Scan Workspace':
...
function generateWorkspaceReport(results, format) {
    const timestamp = new Date().toLocaleString();
