...
export class GuardianChatbot {
    ...
    public async sendMessage(message: string, analysisContext?: any): Promise<ChatMessage> {
        try {
    ...
    private async generateResponse(userMessage: string, session: ChatSession, analysisContext?: any): Promise<ChatMessage> {
    ...
    private async processUserQuery(query: string, analysisContext?: any): Promise<string> {
        try {
            // Input validation
    ...
    private handleQualityQuery(query: string, analysisContext?: any): string {
    ...
    private handleAnalysisQuery(query: string, analysisContext?: any): string {
        if (analysisContext) {
    ...
    private handleFixQuery(_query: string, analysisContext?: any): string {
        try {
            if (analysisContext) {
                const hasVulns = analysisContext.vulnerabilities?.length > 0;
                const hasSecrets = analysisContext.secrets?.length > 0;
                ...
                if (hasVulns) {
                    ...
                    vulns.forEach((vuln: any, i: number) => {
    ...
    private handleGeneralQuery(query: string, analysisContext?: any): string {
        try {
    ...
    private loadSessions(): void {
        try {
            const sessionsData = this.context.globalState.get<any[]>('guardianChatSessions', []);
            const currentSessionId = this.context.globalState.get<string>('guardianCurrentSessionId');
