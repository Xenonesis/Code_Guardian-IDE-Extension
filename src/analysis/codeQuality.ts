...
export class CodeQuality {
    private cache = new Map<string, QualityMetrics>();
    ...
    public async analyzeQuality(code: string): Promise<QualityMetrics> {
    ...
    public getQualityMetrics(code: string): { qualityScore: number; issues: string[] } {
        // Quick return for empty code
        if (!code || code.trim().length === 0) {
    ...
    private cacheResult(key: string, result: QualityMetrics): void {
